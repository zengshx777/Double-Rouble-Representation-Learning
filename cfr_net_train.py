...
tf.app.flags.DEFINE_integer('eb_enable', 1, """Whether Use Entropy Balancing""")
tf.app.flags.DEFINE_string('eb_target', "ATE", """Target population for entropy balancing ATE/ATT/ATC""")
...
tf.app.flags.DEFINE_integer('eb_iter', 20, """Number of iterations for EB step. """)
...
tf.app.flags.DEFINE_string('imb_fun', 'mmd_lin', """Which imbalance penalty to use (mmd_lin/mmd_rbf/mmd2_lin/mmd2_rbf/lindisc/wass/entropy). """)
...
def train(CFR, sess, train_step, D, I_valid, D_test, logfile, i_exp, eb_step=0):
    ...
    for i in range(FLAGS.iterations):
        ...
        ''' Do one step of gradient descent on the parameters for outcome modeling and representations '''
        if FLAGS.eb_enable == 0:
            ...
            if not objnan:
                '''EB Step: Multiple Steps to optimizing'''
                for _ in range(FLAGS.eb_iter):
                    sess.run(eb_step, feed_dict={CFR.x: x_batch, CFR.t: t_batch, \
                        CFR.y_: y_batch, CFR.do_in: FLAGS.dropout_in, CFR.do_out: FLAGS.dropout_out, \
                        CFR.r_alpha: FLAGS.p_alpha, CFR.r_lambda: FLAGS.p_lambda, CFR.p_t: p_treated})
                '''Learn Outcome Model and Representation'''
                sess.run(train_step, feed_dict={CFR.x: x_batch, CFR.t: t_batch, \
                                                CFR.y_: y_batch, CFR.do_in: FLAGS.dropout_in, CFR.do_out: FLAGS.dropout_out, \
        ...
        if i % FLAGS.output_delay == 0 or i==FLAGS.iterations-1:
            ...
            valid_obj = np.nan; valid_imb = np.nan; valid_f_error = np.nan; entropy=np.nan;
            if FLAGS.val_part > 0:
...
def run(outdir):
    ...
    if FLAGS.eb_enable==1:
        ##Train the parameters other than the dual problem
        rep_vars = tf.trainable_variables(scope='Representation_Layer')
        output_vars = tf.trainable_variables(scope='Output_Layer')
        dual_vars = tf.trainable_variables(scope='EntropyBalancing')
        train_step = opt.minimize(CFR.tot_loss,global_step=global_step,var_list = [rep_vars,output_vars])
        #Only Solving the EB problem
        eb_step = opt.minimize(CFR.imb_loss, global_step=global_step, var_list = dual_vars)
    else:
        train_step = opt.minimize(CFR.tot_loss, global_step=global_step)  # type: object
        eb_step = 0# type: object
    ...
    for i_exp in range(1,n_experiments+1):
        ...
        losses, preds_train, preds_test, reps, reps_test = \
            train(CFR, sess, train_step, D_exp, I_valid, \
                D_exp_test, logfile, i_exp, eb_step)
